...
func searchRecipes(_ ingredients: String,_ searchCallBack: @escaping ([[String: Any]]) -> Void) {
    ...
    getRecipes(ingredients, resultSize) { results, error in
       ...
       // print(results)
        ...
        for item in results{
            ...
            let id = Int64(recipeDict["id"] as! Int)
            recipeOverview["id"]  = id
            recipeOverview["title"]     = recipeDict["title"]       as! String
            recipeOverview["calorie"]   = recipeDict["calorie"]    as! String
            recipeOverview["imageURL"]  = recipeDict["imageURL"]       as! String
            ...
            //2) processing needed for storing information(access database)
            //declare additional variables needed for storing the information
            //find information in the dictionary
            //store information in database

            _ = SQLiteDB.instance.storeRecipeToDB(recipeID: id , field: "title", data: recipeDict["title"] as! String)
            _ = SQLiteDB.instance.storeRecipeToDB(recipeID: id , field: "calorie", data: recipeDict["calorie"] as! String)
            _ = SQLiteDB.instance.storeRecipeToDB(recipeID: id , field: "imageURL", data: recipeDict["imageURL"] as! String)
            _ = SQLiteDB.instance.storeRecipeToDB(recipeID: id , field: "servings", data: recipeDict["servings"] as! String)
            _ = SQLiteDB.instance.storeRecipeToDB(recipeID: id , field: "readyInMinutes", data: recipeDict["readyInMinutes"] as! String)
            _ = SQLiteDB.instance.storeRecipeToDB(recipeID: id , field: "preparationMinutes", data: recipeDict["preparationMinutes"] as! String)
            _ = SQLiteDB.instance.storeRecipeToDB(recipeID: id , field: "cookingMinutes", data: recipeDict["cookingMinutes"] as! String)
            ...
            let ingredients = recipeDict["ingredients"] as! [[String:Any]]
            var ingredientList = ""
            for item in ingredients {
                let amountVal = item["amount"] as! Double
            ...
            ingredientList.remove(at: ingredientList.index(before: ingredientList.endIndex))
            
            _ = SQLiteDB.instance.storeRecipeToDB(recipeID: id , field: "ingredients", data: ingredientList )

            
            
            let instructionItem = recipeDict["analyzedInstructions"] as! [[String: Any]]
            for item in instructionItem {
                let steps = item["steps"] as! [[String:Any]]
                var allInstructions = ""
                for s in steps {
                    let numberVal = s["number"] as! Int
                    let number = "\(numberVal)"
                    let step = s["step"] as! String
                    allInstructions += number  + "|" + step + "|"
                }
                
                allInstructions.remove(at: allInstructions.index(before: allInstructions.endIndex))
                _ = SQLiteDB.instance.storeRecipeToDB(recipeID: id , field: "analyzedInstructions", data: allInstructions )
            }
    ...
    for ingredient in ingredientsArray {
        ...
        if(amountVal > 0) {
            ...
            if(relevantItem.listID == -1) {
                var itemGroup = Constants.groups[4]
                for item in pantryItems {
                    if(item.group != "") {
